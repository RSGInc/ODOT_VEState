  ...
  Inp = items(
    item(
      ...
      DESCRIPTION = items(
        ...
        "Proportion of activity in center area type that is served by shared car service (i.e. service competitive with household owned car)",
        "Proportion of activity in inner area type that is served by shared car service (i.e. service competitive with household owned car)",
        "Proportion of activity in outer area type that is served by shared car service (i.e. service competitive with household owned car)",
        "Proportion of activity in fringe area type that is served by shared car service (i.e. service competitive with household owned car)"
      )
    )
  ),
...
AssignCarSvcAvailability <- function(L) {
  ...
  #Make matrix of proportion shared car service by Marea and area type
  PropShdSvc_MaAt <- cbind(
    center = L$Year$Marea$CenterPropShdCarSvc,
    inner = L$Year$Marea$InnerPropShdCarSvc,
    outer = L$Year$Marea$OuterPropShdCarSvc,
    fringe = L$Year$Marea$FringePropShdCarSvc
  )
  rownames(PropShdSvc_MaAt) <- L$Year$Marea$Marea
  #Initialize high car service Bzone vector
  ShdSvcLevel_Bz <- setNames(rep(0L, length(Bz)), Bz)
  ...
  for (ma in Ma) {
    ...
    #Define function to return names of Bzones with shared car service
    getShdSvcBzones <- function(D_df, PropAct) {
      if (PropAct > 0) {
        D_df$PropTotAct <- (D_df$NumHh + D_df$TotEmp) / sum(D_df$NumHh + D_df$TotEmp)
        D1DOrder_ <- rev(order(D_df$D1D))
        CumSumOrder_ <- cumsum(D_df$PropTotAct[D1DOrder_])
    ...
    ShdSvcBzones_ <- do.call(c, sapply(Ax, function(x) {
      getShdSvcBzones(Bx_At_df[[x]], PropShdSvc_MaAt[ma,x])
    }, simplify = FALSE))
    ShdSvcLevel_Bz[ShdSvcBzones_] <- 1L
    rm(BzInMa, Bx, Fields_, Bx_df, Bx_At_df, Ax, HiBzones_, ShdSvcBzones_)
  }
  ...
  Out_ls$Year$Bzone$ShdSvcAvail <- ShdSvcLevel_Bz
